...
namespace RAG
{
    public class Utilities
    {
        ...
        public static List<ChatRequestMessage> history = new List<ChatRequestMessage>();

        public static List<string> followOnQuestionList = new List<string>();
        ...
        /// <returns></returns>
        public static string CallGPT(string prompt, string? profileText = null, bool displayAnswer = false, bool displayPrompt = false, bool addToHistory = true, int followOnQuestions = 0, bool autoAskFollowOns = false)
        {
            // Setup
            // ---------------------------------------------------------------
            ...
            // Add history into the request to provide context
            if (history != null)
            {
                foreach (var item in history)
                {
                    cr.messages.Add(new ChatRequestMessage() { content = item.content, role = item.role });
                }
            }

            // If caller asked for follow-on questions, update the prompt
            // to request them. Allow a maximum of 3.
            string followOnQuestionHeading = "Follow-on Questions:";
            if (followOnQuestions > 0)
            {
                prompt += $". After answering this question, please suggest the {Math.Min(followOnQuestions,3)} most useful follow-on questions one might ask. Format the list of follow-on questions as follows: {followOnQuestionHeading} 1. follow-on question #1; 2. follow-on question #2; etc.";
            }

            cr.messages.Add(new ChatRequestMessage() { content = prompt, role = "user" });
            string answer = openAiAPI.Chat(cr).Result.choices[0].message.content;

            // If the caller wants to automatically ask the follow-on questions,
            // do so here and append the answers to the reply.
            string matchStringPattern = @"^\d\.";
            string fixStringPattern = @"^\d\.\s*";
            if (autoAskFollowOns)
            {
                string foQuestionsText = answer.Substring(answer.IndexOf($"{followOnQuestionHeading} ") + followOnQuestionHeading.Length + 1);
                answer = answer.Substring(0, answer.IndexOf($"{followOnQuestionHeading} ")-1) + "\nFollow-on questions with answers: \n";
                string[] foQuestions = foQuestionsText.Split(new char[] { ',', ';', '\n', '\r' });

                for (int i = 0; i < foQuestions.Length; i++)
                {
                    if (Regex.IsMatch(foQuestions[i], matchStringPattern))
                    {
                        // Use Regex.Replace to remove the matched pattern
                        string foQuestion = Regex.Replace(foQuestions[i], fixStringPattern, string.Empty);
                        string foAnswer = Utilities.CallGPT(prompt: foQuestion, profileText: null, displayAnswer: false, addToHistory: false, followOnQuestions: 0, autoAskFollowOns: false);
                        answer += $"\n\n{i}. {foQuestion}\n\n   {foAnswer}";
                    }
                }
            }
            else if (followOnQuestions > 0)
            {
                // Extract the follow-on questions and make them available to the seeker
                string foQuestionsText = answer.Substring(answer.IndexOf($"{followOnQuestionHeading} ") + followOnQuestionHeading.Length + 1);
                string[] foQuestions = foQuestionsText.Split(new char[] { ',', ';', '\n', '\r' });
                followOnQuestionList = new List<string>();
                for (int i = 0; i < foQuestions.Length; i++)
                {
                    if (Regex.IsMatch(foQuestions[i], matchStringPattern))
                    {
                        // Use Regex.Replace to remove the matched pattern
                        string foQuestion = Regex.Replace(foQuestions[i], fixStringPattern, string.Empty);
                        followOnQuestionList.Add(foQuestion);
                    }
                }
            }
            ...
            if (addToHistory)
            {
                history.Add(new ChatRequestMessage() { content = prompt, role = "user" });
                history.Add(new ChatRequestMessage() { content = answer, role = "assistant" });
            }
        ...
        public static string FixUpSql(string sql)
        {
            sql = sql.Replace("```", "");
        ...
        public static string GetDataSource(string question)
        {
            ...
            string answer = CallGPT(prompt: prompt, addToHistory: false, followOnQuestions: 0);
            return answer;
        }
        ...
        public static void QuerySQL(string question)
        {
            ...
            try
            {
                ...
                answer = CallGPT(prompt, null, false, false, followOnQuestions:2);
        ...
        public static string FixUpStock(string stock)
        {
        ...
        static string prevCompany = "Unknown";
        ...
        public static void QueryStock(string question)
        {
            try
            {
                ...
                string company = FixUpStock(CallGPT(prompt));
                if (company.Contains("Unknown"))
                {
                    company = prevCompany;
                }
                else
                ...
                string answer = CallGPT(prompt, followOnQuestions: 2);
        ...
        public static void QueryVector(string question)
        {
            ...
            if (findResult == null || findResult.data.documents[0].similarity < MIN_DATA_CONFIDENCE)
            {
                ...
                // Call GPT to get the answer
                var answer = CallGPT(prompt, followOnQuestions: 2);
            ...
            else
            {
                ...
                foreach (var keyword in keywords)
                {
                    ...
                    {
                        // Call GPT to get the answer
                        answer = CallGPT(question, followOnQuestions: 2);
        ...
        static string prevCity = "Unknown";
        static string prevState = "CA";
        ...
        public static void QueryWeather(string question)
        {
            ...
            try
            {
                ...
                // #2: Call GPT to get the answer
                loc = CallGPT(prompt: prompt, addToHistory: false);
                string city = loc.Split(';').First() == "Unknown" ? prevCity : loc.Split(';').First();
                if (city != "Unknown") prevCity = city;

                string state = loc.Split(';').Last() == "Unknown" ? prevState : loc.Split(';').Last();
                if (state != "Unknown") prevState = state;
                ...
                answer = CallGPT(prompt, followOnQuestions: 2);
